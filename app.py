...
if st.session_state.get("logged_in"):
    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        # Ajustado para filtrar exatamente o mesmo mês e ano
        data_inicio_cadastro = datetime(data_inicio.year - 1, data_inicio.month, 1)
        if data_inicio.month == 12:
            data_fim_cadastro = datetime(data_inicio.year, 1, 1) - timedelta(seconds=1)
        else:
            data_fim_cadastro = datetime(data_inicio.year - 1, data_inicio.month + 1, 1) - timedelta(seconds=1)
        ...
        if tipo_filtro != "Todos":
            lista_empresas = [e for e in lista_empresas if any(ext.get("tipo") == tipo_filtro for
                                                               ext in e.get("extintores", []))]
