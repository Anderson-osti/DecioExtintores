...
if st.session_state.get("logged_in"):
    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        mes_filtro = st.date_input("Selecionar mês do vencimento", value=datetime.today())

        # Corrige: filtro baseado no cadastro que vence um ano depois
        data_inicio = datetime(mes_filtro.year - 1, mes_filtro.month, 1)
        if mes_filtro.month == 12:
            data_fim = datetime(mes_filtro.year - 1 + 1, 1, 1) - timedelta(days=1)
        else:
            data_fim = datetime(mes_filtro.year - 1, mes_filtro.month + 1, 1) - timedelta(days=1)
        ...
        if tipo_filtro != "Todos":
            lista_empresas = [e for e in lista_empresas if any(ext.get("tipo") == tipo_filtro for ext in e.get("extintores", []))]
