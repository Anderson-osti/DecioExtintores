...
if st.session_state.get("logged_in"):
    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        filtro = {"usuario": st.session_state["usuario_logado"]}
        if cidade_filtro != "Todas":
            filtro["cidade"] = {"$regex": f"^{cidade_filtro}$", "$options": "i"}

        empresas = list(companies_collection.find(filtro))

        # Verifica vencimento (1 ano após cadastro)
        empresas_vencimento = []
        for emp in empresas:
            vencimento = emp["data_cadastro"] + timedelta(days=365)
            if data_inicio <= vencimento.date() <= data_fim:
                if tipo_filtro == "Todos" or any(ext.get("tipo") == tipo_filtro for ext in emp.get("extintores", [])):
                    empresas_vencimento.append(emp)

        st.subheader("Empresas encontradas:")
        st.write(f"Total: {len(empresas_vencimento)}")
        ...
        def gerar_pdf(empresas_pdf):
            ...
            for emp in empresas_pdf:
                ...
                # Agrupar por tipo/capacidade
                resumo = defaultdict(int)
                for ext in emp.get("extintores", []):
                    chave = f"{ext['tipo']} {ext['capacidade']}"
                    resumo[chave] += ext["quantidade"]

                for k, v in resumo.items():
                    pdf.cell(0, 6, f"- {v}x {k}", ln=True)

                if emp.get("mangueiras"):
        ...
        if st.button("Baixar Relatório em PDF"):
            pdf_bytes = gerar_pdf(empresas_vencimento)
