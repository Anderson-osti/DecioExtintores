...
if st.session_state.get("logged_in"):
    ...
    pagina_selecionada = st.sidebar.radio("Ir para",
                                          ["Cadastro de Empresa", "Empresas Cadastradas", "Relatório de Vencimento"])
    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        if cidade_filtro != "Todas":
            filtro_base["cidade"] = {"$regex": f"^{cidade_filtro}$", "$options": "i"}

        empresas_filtradas = list(companies_collection.find(filtro_base))

        if tipo_filtro != "Todos":
            empresas_filtradas = [
                e for e in empresas_filtradas
                if any(ext["tipo"] == tipo_filtro for ext in e.get("extintores", []))
            ]

        empresas_vencendo = [
            e for e in empresas_filtradas
            if data_inicio <= (e["data_cadastro"] + timedelta(days=365)).date() <= data_fim
        ]

        st.subheader("Empresas com extintores vencendo:")
        st.write(f"Total: {len(empresas_vencendo)}")
        ...
        if st.button("Baixar Relatório em PDF"):
            pdf_bytes = gerar_pdf(empresas_vencendo)
