...
if st.session_state.get("logged_in"):
    ...
    if pagina == "Cadastro de Empresa":
        ...
        for tipo in tipos_extintores:
            if st.checkbox(f"Adicionar {tipo}"):
                capacidade = st.multiselect(f"Capacidade ({tipo})", capacidades_extintores[tipo])
                if capacidade:
                    extintores_selecionados[tipo] = capacidade
        ...
        empresa_selecionada = st.selectbox("Selecione uma Empresa", [""] + [e["nome"] for e in empresas])
        ...
        if empresa_selecionada:
            ...
            st.write(f"🧯 Extintores: {empresa['extintores']}")
            st.write(f"🚿 Mangueiras: {empresa['mangueiras']}")
