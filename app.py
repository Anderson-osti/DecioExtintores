    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        if empresa_selecionada:
            ...
            if empresa:
                ...
                st.write(f"🗓️ Cadastro: {empresa['data_cadastro'].strftime('%d/%m/%Y')}")
        ...
        # Vencimento 1 ano após o cadastro
        data_inicio_cadastro = datetime.combine(data_inicio - timedelta(days=365), datetime.min.time())
        data_fim_cadastro = datetime.combine(data_fim - timedelta(days=365), datetime.max.time())

        filtro_busca = {
            "usuario": st.session_state["usuario_logado"],
            "data_cadastro": {
                "$gte": data_inicio_cadastro,
                "$lte": data_fim_cadastro
            }
        }
