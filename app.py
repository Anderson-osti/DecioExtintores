...
if st.session_state.get("logged_in"):
    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        # Corrigido: Buscar empresas cadastradas há 1 ano
        vencimento_inicio = datetime(mes_ano.year - 1, mes_ano.month, 1)
        if mes_ano.month == 12:
            vencimento_fim = datetime(mes_ano.year, 1, 1) - timedelta(seconds=1)
        else:
            vencimento_fim = datetime(mes_ano.year - 1, mes_ano.month + 1, 1) - timedelta(seconds=1)

        filtro_busca = {
            "usuario": st.session_state["usuario_logado"],
            "data_cadastro": {
                "$gte": vencimento_inicio,
                "$lte": vencimento_fim
            }
        }
