...
def calcular_resumo_extintores(empresas):
    resumo = defaultdict(int)
    for emp in empresas:
        for ext in emp.get("extintores", []):
            chave = f"{ext.get('tipo', '')} {ext.get('capacidade', '')}"
            resumo[chave] += ext.get("quantidade", 0)
    return dict(resumo)
...
if st.session_state.get("logged_in"):
    ...
    pagina_selecionada = st.sidebar.radio("Ir para", ["Cadastro de Empresa", "Empresas Cadastradas", "Relatório de Vencimento"])
    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        if tipo_filtro != "Todos":
            empresas_filtradas = [e for e in empresas_filtradas if any(ext.get("tipo") == tipo_filtro for ext in e.get("extintores", []))]

        st.subheader("Empresas encontradas:")
        st.write(f"Total: {len(empresas_filtradas)}")

        resumo_extintores = calcular_resumo_extintores(empresas_filtradas)
        if resumo_extintores:
            st.subheader("Resumo de Extintores:")
            for tipo_capacidade, qtd in resumo_extintores.items():
                st.markdown(f"- {tipo_capacidade}: **{qtd} unidade(s)**")
        ...
        def gerar_pdf(empresas_pdf):
            ...
            pdf.cell(0, 8, "Resumo de Extintores:", ln=True)

            resumo = calcular_resumo_extintores(empresas_pdf)
            pdf.set_font("Arial", "", 10)
            for tipo_cap, qtd in resumo.items():
                pdf.cell(0, 6, f"- {tipo_cap}: {qtd} unidade(s)", ln=True)

            return pdf.output(dest='S').encode('latin1')

        if st.button("Baixar Relatório em PDF"):
            pdf_bytes = gerar_pdf(empresas_filtradas)
            st.download_button("Clique aqui para baixar", data=pdf_bytes, file_name="relatorio_vencimento.pdf", mime="application/pdf")
