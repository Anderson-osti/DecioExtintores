    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        def gerar_pdf(empresas_pdf):
            ...
            for emp in empresas_pdf:
                linha = f"{emp['nome']} | {emp['cidade']} | {emp['telefone']} | {emp['data_cadastro'].strftime('%d/%m/%Y')}"

                descricoes = []
                for ext in emp.get("extintores", []):
                    chave = f"{ext['tipo']} {ext['capacidade']}"
                    totais[chave] += ext["quantidade"]
                    descricoes.append(f"{ext['quantidade']}x {chave}")

                for tipo_m, qtd_m in emp.get("mangueiras", {}).items():
                    descricoes.append(f"{qtd_m}x Mangueira {tipo_m}")
