...
if st.session_state.get("logged_in"):
    ...
    if pagina_selecionada == "Cadastro de Empresa":
        ...
        fim_mes = datetime(mes_ano.year, mes_ano.month + 1, 1) - timedelta(seconds=1) if mes_ano.month < 12 else datetime(mes_ano.year + 1, 1, 1) - timedelta(seconds=1)
        ...
        for empresa in empresas_usuario:
            ...
            if inicio_mes <= data_vencimento <= fim_mes:
                if cidade_filtro == "Todas" or empresa.get("cidade", "").lower() == cidade_filtro.lower():
                    if tipo_filtro == "Todos" or any(ext.get("tipo") == tipo_filtro for ext in empresa.get("extintores", [])):
